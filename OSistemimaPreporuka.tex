...
% \title{Sistemi preporuka i uticaj na formiranje stavova\\ \small{Seminarski rad u okviru kursa\\Metodologija stručnog i naučnog rada\\ Matematički fakultet}}
...
\appendix
\bibliography{OSistemimaPreporuka} 
\bibliographystyle{plain}

\end{document}

% Problem formiranja preporuka se obično formuliše na sledeći način: ukoliko je dat skup korisnika $C$ i skup predmeta $S$, definiše se funkcija upotrebljivosti $u : C \times S \to \mathbb{R}$. Zadatak sistema preporuka je da za svakog korisnika $c \in C$ odredi $k$ predmeta $s \in S$ tako da im je upotrebljivost najveća \cite{burke_recommender_2011}.

% Ako je $m$ broj korisnika i $n$ broj predmeta, upotrebljivost možemo predstaviti u obliku matrice $R$ formata $(m, n)$ koja se zove matrica ocena (eng.~{\em rating matrix}). Matrica $R$ je retka matrica u kojoj su zapisane poznate vrednosti upotrebljivosti koje je sistem izračunao na osnovu aktivnosti korisnika, dok ostale vrednosti sistem procenjuje na osnovu poznatih. Ovo posmatranje problema se naziva problem popunjavanja matrice (eng.~{\em matrix completion}).
